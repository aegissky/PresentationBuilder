...
(function () {
  ...
  /* ── 페이지 목록 ─────────────────────────────────────────────
   * PB-MOD-1 W2: PAGES 데이터는 인스턴스별 nav-data.js에서 외부 주입.
   *   nav-data.js: window.PRES_NAV_DATA = { pages: [{ group, items: [{ file, label }] }, ...] };
   * fallback: 주입 없으면 빈 배열 (네비 패널은 표시되나 페이지 드롭다운만 비어 보임).
   * ──────────────────────────────────────────────────────────── */
  const PAGES = (typeof window !== 'undefined'
                  && window.PRES_NAV_DATA
                  && Array.isArray(window.PRES_NAV_DATA.pages))
    ? window.PRES_NAV_DATA.pages
    : [];
